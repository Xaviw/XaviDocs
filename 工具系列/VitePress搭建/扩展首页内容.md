...
:::tip 提示
因为`vitepress`会读取配置文本中的关键字，所以例如`HTML`标签和`import.meat.env`等字符,需要处理过后再存储

本文档功能无需得到完整的页面数据，所以直接在`readPages`中就对内容进行了完整的处理。如果需要完整的页面数据，可以只再`readPages`中处理`import`、`export`、`HTML标签`字符，后续在特定位置再针对处理数据
:::
