      ...
      pages.map((item) => {
        ...
        return {
          ...
          details: item.content
            .slice(0, 200) // 只保留一个安全长度，减少正则匹配时间
            .replace(/#* [\S]+?\s/g, '') // 去掉最影响展示的标题标签
            .replace(/<[^>]+>/g, ''), // 删除HTML标签，因为details使用的v-html渲染，源码位置vitepress\src\client\theme-default\components\VPFeature.vue
          link: item.path,
