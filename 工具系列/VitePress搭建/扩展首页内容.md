...
:::tip 提示
本文档创作过程中发现不能在`config`配置中含有`import.meta.env`字样，在向官方提`issue`后得到回复这并不算一个bug，建议通过`\0`插入空字符解决因为

同样会被识别的还有部分`HTML`标签，考虑到`export`可能也会有影响，所以这里一并处理
:::
...
另外`VPFeature`组件展示内容是使用的`v-html`，前文`readPages`中刚好处理了`HTML`标签，所以无需再处理
:::
